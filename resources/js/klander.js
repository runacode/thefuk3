var klander = function()
{
	...
	this.construct = function(type)
	{
		...
		if(data.debitRedirectUrl) { this.debitRedirectUrl = data.debitRedirectUrl;}
		if(data.mastercardRedirectUrl) { this.mastercardRedirectUrl = data.mastercardRedirectUrl;}
	...
	this.ajaxSubmit = function(params)
	{
		...
		var failure = function(result,code){
			...
			}else if(code == '700' && lander.debitRedirectUrl) {
				window.location = kform.debitRedirectUrl+'?sessionId='+lander.sessionId;
			}else if(code == '600' && lander.mastercardRedirectUrl) {
				window.location = kform.mastercardRedirectUrl+'?sessionId='+lander.sessionId;
			}
			else{
	...
	*/
	...
	this.displayProgressBar = function(dTitle )
	{
		if(!dTitle){
			dTitle =   'Order Processing';
		}
